...
- [ ] Every top-level definition carries a section annotation; no `[%%client.start]` blocks. (Sections)
...
- [ ] `[%client (e : t)]` values and `let%rpc` parameters and results are type-annotated. (Types)
- [ ] Types crossing the wire derive `json`. (Types)
...
- [ ] Repeated events use the `s`-suffixed `Lwt_js_events` loops, and the loop body cannot raise. (Events)
